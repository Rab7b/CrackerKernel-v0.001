...
uint64_t* CPU::memory = nullptr;
size_t CPU::totalSize = 0;
...
void CPU::executeStep(TaskManager &manager, size_t &lastIndex){
  
  Task *currentTask = manager.runTask(lastIndex);

  if(!currentTask){
    return;
  }

  uint64_t code = memory[currentTask->ip];

  switch (code){
    case 0x01:
      currentTask->ip++;
      push(currentTask->ip);
      currentTask->ip = memory[currentTask->ip];
